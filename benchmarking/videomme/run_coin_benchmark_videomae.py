"""
Run COIN benchmark with VideoMAE V2 + Qwen2.5-1.5B - FULL DATASET
...
print("Starting VideoMAE Full Benchmark...", flush=True)
...
def main():
    print("\n" + "="*80, flush=True)
    print("TEMPORALBENCH COIN FULL BENCHMARK - VideoMAE V2 + Qwen2.5-1.5B", flush=True)
    print("="*80, flush=True)
    ...
    print(f"Testing on all {len(videos_qa)} videos", flush=True)
    ...
    results_file = output_dir / f"results_videomae_full_{timestamp}.json"
    predictions_file = output_dir / f"predictions_videomae_full_{timestamp}.jsonl"
    ...
    # Summary
    print(f"\n{'='*80}")
    print(f"FULL BENCHMARK RESULTS - VideoMAE V2 + Qwen2.5-1.5B")
    print(f"{'='*80}")
